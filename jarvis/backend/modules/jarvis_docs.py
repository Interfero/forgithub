...
from modules.memory_store import TECH_DOC_FILENAME
...
def _migrate_tech_doc_from_conscious() -> None:
    """Перенос устаревшего расположения (conscious → unconscious)."""
    from modules.memory_store import CONSCIOUS_DIR, UNCONSCIOUS_DIR

    old = CONSCIOUS_DIR / TECH_DOC_FILENAME
    if not old.is_file():
        return
    UNCONSCIOUS_DIR.mkdir(parents=True, exist_ok=True)
    new = UNCONSCIOUS_DIR / TECH_DOC_FILENAME
    if not new.exists():
    ...
    try:
        ...
        if old_size > new_size + 256 or old_mtime > new_mtime + 1:
...
def ensure_tech_documentation() -> Path:
    """Создать/обновить Техдокументация.txt в бессознательном из bundle."""
    from modules.memory_store import UNCONSCIOUS_DIR, add_file

    _migrate_tech_doc_from_conscious()
    UNCONSCIOUS_DIR.mkdir(parents=True, exist_ok=True)
    dst = UNCONSCIOUS_DIR / TECH_DOC_FILENAME
    src = _bundle_doc
    if src.is_file():
        if not dst.exists() or src.stat().st_mtime > dst.stat().st_mtime:
            shutil.copy2(src, dst)
    elif not dst.exists():
        add_file("unconscious", TECH_DOC_FILENAME, _minimal_tech_doc().encode("utf-8"))
    try:
...
def read_tech_documentation(max_chars: int = 24000) -> str:
    ...
    row = read_file("unconscious", TECH_DOC_FILENAME)
...
def get_tech_doc_prompt_hint() -> str:
    ...
    hint = (
        ...
        "Файл «Техдокументация.txt» в **Бессознательном** описывает, из чего состоит Jarvis "
...
def probe_tech_doc_part() -> tuple[str, str, str]:
    """Строка отчёта «Проверка систем» для Техдокументация.txt."""
    path = ensure_tech_documentation()
    doc = read_tech_documentation(max_chars=400)
    if not path.is_file():
        return ("warn", "Техдокументация", "Нет Техдокументация.txt в бессознательном")
    if not doc.strip():
        return ("warn", "Техдокументация", "Файл пустой — проверьте data/memory/unconscious/")
    size_kb = max(1, path.stat().st_size // 1024)
    with _last_doc_read_lock:
        ts = _last_doc_read_at
    detail = f"В бессознательном · {size_kb} КБ · в промпт до 16 000 симв."
    if ts:
        detail += f" · перечитано {ts}"
    return ("ok", "Техдокументация", detail)
...
def _minimal_tech_doc() -> str:
    return (
        ...
        "Техдокументация.txt в папку memory/unconscious.\n"
    )


def read_conscious_excluding_tech_doc(max_chars: int = 4000) -> str:
    """Сознательное без служебных файлов Jarvis."""
    from modules.memory_store import (
        ALLOWED_EXT,
        CONSCIOUS_DIR,
        STEREOTYPES_FILENAMES,
        TECH_DOC_FILENAME,
        is_internal_conscious_file,
    )

    CONSCIOUS_DIR.mkdir(parents=True, exist_ok=True)
    parts: list[str] = []
    total = 0
    for path in sorted(CONSCIOUS_DIR.iterdir()):
        if not path.is_file() or path.suffix.lower() not in ALLOWED_EXT:
            continue
        name = path.name
        if (
            is_internal_conscious_file(name)
            or name == TECH_DOC_FILENAME
            or name in STEREOTYPES_FILENAMES
        ):
            continue
        try:
