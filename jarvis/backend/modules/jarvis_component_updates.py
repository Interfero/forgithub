"""
Автообновление компонентов Jarvis (pip + браузеры Playwright).
...
def _state_path():
    ...
    return user_data_dir() / "jarvis_component_updates.json"
...
def run_components_update(
    *,
    ...
    if force_pip:
        pip_args.append("-U")
    pip_args.extend(COMPONENT_PIP_PACKAGES)
...
def maybe_run_components_update_async(*, force: bool = False) -> bool:
        ...
        with _UPDATE_LOCK:
            ...
            try:
                ...
                if not probe_internet()[0]:
                    return
                run_components_update(force_browsers=force, force_pip=True)
            except Exception as e:
                _log.warning("component update: %s", e)

    threading.Thread(
        target=_work,
        daemon=True,
        name="jarvis-component-updates",
    ).start()
    return True


def components_update_status() -> dict[str, Any]:
    state = _load_state()
