...
STEREOTYPES_FILENAME = "\u0421\u0442\u0435\u0440\u043e\u0442\u0438\u043f\u044b.txt"
STEREOTYPES_FILENAME_ALT = "Stereotypes.txt"
STEREOTYPES_FILENAMES = frozenset({STEREOTYPES_FILENAME, STEREOTYPES_FILENAME_ALT})
...
# В общий блок бессознательного не включается — подмешивается отдельно (jarvis_docs, web_stereotypes).
INTERNAL_UNCONSCIOUS_FILES = frozenset(
    {TECH_DOC_FILENAME, STEREOTYPES_FILENAME, STEREOTYPES_FILENAME_ALT}
)
...
def _migrate_stereotypes_to_unconscious() -> None:
    """Перенос Стерeотипы.txt из сознательного в бессознательное (не в каждый промпт)."""
    CONSCIOUS_DIR.mkdir(parents=True, exist_ok=True)
    UNCONSCIOUS_DIR.mkdir(parents=True, exist_ok=True)
    for name in STEREOTYPES_FILENAMES:
        src = CONSCIOUS_DIR / name
        dst = UNCONSCIOUS_DIR / name
        if not src.is_file():
            continue
        if not dst.is_file():
    ...
    _merge_duplicate_stereotypes_files()


def _merge_duplicate_stereotypes_files() -> None:
    """Стереотипы.txt (опечатка) и Стереотипы.txt — один канонический файл."""
    canonical = UNCONSCIOUS_DIR / STEREOTYPES_FILENAME
    alt_names = ("Стереотипы.txt", "Stereotypes.txt")
    if not canonical.is_file():
        for alt in alt_names:
            p = UNCONSCIOUS_DIR / alt
            if p.is_file():
                shutil.copy2(p, canonical)
                break
    try:
        from modules.web_stereotypes import _merge_bundled_stereotypes

        if canonical.is_file():
            _merge_bundled_stereotypes(canonical)
    except Exception:
        pass


def _purge_stereotypes_from_conscious() -> None:
    """Стерeотипы не должны появляться в сознательном (только файлы пользователя из чата)."""
    CONSCIOUS_DIR.mkdir(parents=True, exist_ok=True)
    for name in STEREOTYPES_FILENAMES:
        path = CONSCIOUS_DIR / name
        if path.is_file():
    ...
    try:
        ...
        jarvis_db.init_db()
        for name in STEREOTYPES_FILENAMES:
            jarvis_db.delete_file("conscious", name)
    except Exception:
        pass
...
def _ensure_browser_window_rules_file() -> None:
    dst = UNCONSCIOUS_DIR / "browser_window_rules.txt"
    if dst.exists():
        return
    src = (
        Path(__file__).resolve().parent.parent
...
def _ensure_dirs() -> None:
    ...
    _migrate_stereotypes_to_unconscious()
    _purge_stereotypes_from_conscious()
    ...
    if not default_unconscious.exists():
        default_unconscious.write_text(
            ...
            "По умолчанию отвечай коротко и по делу (1–3 предложения). "
    ...
    _ensure_browser_window_rules_file()
...
def read_all_text(store: str, max_chars: int = 12000) -> str:
    exclude = INTERNAL_UNCONSCIOUS_FILES if store == "unconscious" else frozenset()
    try:
        from modules import jarvis_db

        jarvis_db.init_db()
        text = jarvis_db.read_store_text(
            store,
            max_chars=max_chars,
