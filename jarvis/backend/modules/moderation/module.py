...
_BLOCK_ESCALATION_RESPONSE = (
    "Шеф, после нескольких грубых сообщений я приостанавливаю обычный диалог на ~30 минут. "
    "Когда будете готовы общаться уважительно — продолжим."
)
        ...
        if raw_action == "OK" and self.ml_enabled:
            try:
                ...
                if ml_score >= self.ml_threshold:
                    ...
                    if not response:
                        response = (
                            "Похоже, в сообщении агрессивный тон. "
                            "Давайте без этого — я здесь, чтобы помочь."
                        )
            except Exception:
