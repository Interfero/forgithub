...
class CustomSkills:
    ...
    @staticmethod
    def read_document_file(path: str, max_chars: str = "50000") -> str:
        """Прочитать PDF, docx или текстовый файл с диска."""
        from modules.document_tools import format_read_result, read_document

        try:
            data = read_document(path, max_chars=int(max_chars or 50_000))
            return format_read_result(data)
        except Exception as e:
            return f"Ошибка чтения: {e}"

    @staticmethod
    def convert_document_file(source: str, target_format: str, output_path: str = "") -> str:
        """Конвертация: изображение↔PDF, текст/docx→PDF, PDF→txt/png/jpg."""
        from modules.document_tools import convert_document, format_convert_result

        try:
            res = convert_document(source, target_format, output_path=output_path or "")
            return format_convert_result(res)
        except Exception as e:
    ...
    def compile_and_run_code(
        ...
        """Скомпилировать и/или запустить код (python, js, java, c, cpp, go, rust…)."""
        from modules.code_runner import compile_and_run, format_run_result
        ...
        try:
            res = compile_and_run(
                ...
                language=language,
                filename=filename,
                run_after_compile=str(run).lower() not in ("0", "false", "нет", "no"),
            )
            return format_run_result(res)
        except Exception as e:
            return f"Ошибка: {e}"

    @staticmethod
    def document_formats_help() -> str:
        from modules.document_tools import supported_formats_help

        return supported_formats_help()

    @staticmethod
    def code_languages_help() -> str:
        from modules.code_runner import supported_languages_help

        return supported_languages_help()
