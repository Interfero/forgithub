...
def _google_search_sync(query: str, max_results: int = 10) -> list[dict[str, str]]:
    ...
    if not _is_ready():
        ...
        st = chromium_browser_status(force_refresh=True)
        if not st.get("ready"):
            raise RuntimeError(st.get("detail") or "Chromium не готов")
    ...
    try:
        url = (
            ...
            f"q={quote_plus(q)}&hl=ru&num={n}&pws=0"
        )
        ...
        raw = page.evaluate(
            """(limit) => {
              const out = [];
              const seen = new Set();
              const nodes = document.querySelectorAll('div.g, div[data-sokoban-container], div.MjjYud');
              for (const el of nodes) {
                const a = el.querySelector('a[href^="http"]');
                if (!a) continue;
                const href = a.href;
                if (!href || seen.has(href)) continue;
                if (/google\\.(com|ru)\\/search|webcache|accounts\\.google/.test(href)) continue;
                const h3 = el.querySelector('h3');
                const title = (h3 && h3.innerText) || a.innerText || '';
                let snippet = '';
                for (const sel of ['div.VwiC3b', 'div[data-snf]', 'span.aCOpRe', 'div.IsZvec']) {
                  const sn = el.querySelector(sel);
                  if (sn && sn.innerText) { snippet = sn.innerText; break; }
                }
...
def google_search_results(query: str, max_results: int = 10) -> list[dict[str, str]]:
    """Google SERP — потокобезопасно через executor Chromium."""
    fut = _EXECUTOR.submit(_google_search_sync, query, max_results)
