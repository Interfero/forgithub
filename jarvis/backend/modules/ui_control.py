...
def get_telegram_fields_snapshot() -> dict[str, Any]:
    ...
    return {
        "bot_token": {
            ...
            "display": cfg.get("bot_token") or "(не задан — введите в Настройках → Коннектор Телеграм)",
...
def get_connectors_live_context() -> str:
    ...
    return (
        ...
        "Обе панели в **Настройках** приложения Jarvis (раздел «Коннектор Телеграм» и «Коннектор Авито»).\n\n"
        "**1) Коннектор Телеграм** — ваш BotFather-бот: токен, прокси, bot_logic.json, "
...
def set_telegram_field(field: str, value: str, *, persist: bool = True) -> str:
    ...
    if field in ("bot_token", "token", "токен"):
        ...
        return "Токен подставлен в поле «Токен бота (BotFather)» в Настройках — нажмите «Сохранить токен»."
...
def execute_ui_click(action: str) -> str:
    ...
    if action in ("expand_telegram", "telegram_panel", "telegram"):
        queue_ui_command({"action": "expand_panel", "panel": "telegram"})
        return "Развёрнут раздел «Коннектор Телеграм» в Настройках."
