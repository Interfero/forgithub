"""
Очистка markdown, утечек промпта и анти-галлюцинаций; краткость — через промпт, не обрезку.
"""
...
_FAKE_USER_TURN = re.compile(
    r"^(?:"
    r"Я\s+хотел\s+(?:узнать|спросить|бы\s+)|"
    r"Какие\s+у\s+меня\s+варианты|"
    r"Мне\s+очень\s+приятно\s+слышать|"
    r"Мне\s+нужн(?:а|о|ы)\s+(?:помощь|с\s+)|"
    r"Может\s+быть,\s+у\s+вас\s+есть|"
    r"Помогите\s+мне\s+|"
    r"У\s+меня\s+есть\s+(?:конкретн|вопрос|проблема|задача)|"
    r"I\s+need\s+help|"
    r"Can\s+you\s+help\s+me"
    r")",
    re.I | re.M,
)
_HALLUCINATED_USER_NEED = re.compile(
    r"(?:^|[.!?…]\s+)(?:"
    r"Мне\s+нужн(?:а|о|ы)\s+(?:помощь|с\s+)|"
    r"У\s+меня\s+есть\s+(?:вопрос|проблема|задача)|"
    r"Я\s+хотел\s+бы\s+(?:узнать|спросить|получить)|"
    r"Помогите\s+мне\s+|"
    r"I\s+need\s+help\s+with|"
...
_IDENTITY_LOOP_RE = re.compile(
    r"я\s*[-—]?\s*jarvis[^.!?\n]{0,120}(?:виртуальн|ассистент|помощник|интеллект)",
    re.I,
)
_USER_PROFANITY_ECHO = re.compile(
    r"(?:извините|простите|мне\s+кажется|вы\s+ошиблись|не\s+уверен)[^.!?\n]{0,80}"
    r"(?:хуй|пизд|бля|еб|ёб)",
    re.I,
)
...
def skips_post_brevity(user_text: str) -> bool:
    ...
    try:
        ...
        if is_general_capabilities_question(user_text):
            return True
        from modules.jarvis_capabilities import is_answer_length_meta_question

        if is_answer_length_meta_question(user_text):
            return True
    except Exception:
        pass
    low = (user_text or "").lower()
...
def reply_max_tokens(user_text: str, *, cloud: bool = False) -> int:
    """Достаточно для полного короткого ответа; краткость — в промпте, не обрезка."""
    if needs_long_reply(user_text) or skips_post_brevity(user_text):
        return 4096 if cloud else 3072
    if user_wants_brief(user_text) or user_question_is_short(user_text):
        return 1024 if cloud else 768
    return 1536 if cloud else 1200
...
def followup_brevity_hint(user_text: str) -> str:
    ...
    if user_question_is_short(user_text):
        return (
            "\n\n[Формат ответа] Кратко (1–3 предложения или до 4 пунктов списка с текстом). "
            "Допиши список до конца. Без воды, без «---», без реплик от имени Шефа."
        )
    if count_user_sentences(user_text) >= 2:
        return (
...
def strip_hallucinated_user_turns(text: str) -> str:
    """Убрать придуманные реплики Шефа (модель «продолжает диалог» сама)."""
    s = (text or "").strip()
    if not s:
        return s
    m = _HALLUCINATED_USER_NEED.search(s)
    if m and m.start() > 24:
        s = s[: m.start()].strip()
    ...
    for part in parts[1:]:
        ...
        if not pl:
            continue
        if _HALLUCINATED_USER_NEED.match(pl) or _FAKE_USER_TURN.match(pl):
            break
        if re.match(
            r"^(?:мне\s+нужн|у\s+меня\s+есть|я\s+хотел|помогите|"
            r"could\s+you\s+help|i\s+need\s+help)",
            pl,
            re.I,
        ):
            break
...
def strip_consumer_assistant_bleed(text: str, user_text: str = "") -> str:
    ...
    s = re.sub(
        r"(?:как\s+я\s+могу\s+помочь|чем\s+могу\s+вам\s+помочь)[^.!?\n]*[.!?]?",
        "",
        s,
        flags=re.I,
    )
...
def strip_identity_spam(text: str) -> str:
    """Убрать повторяющиеся блоки «Я — Jarvis, виртуальный ассистент…»."""
    blocks = re.split(r"\n\s*\n", (text or "").strip())
    kept: list[str] = []
    seen_identity = False
    for block in blocks:
        ...
        if _IDENTITY_LOOP_RE.search(b):
            ...
            kept.append(b)
            continue
        if _USER_PROFANITY_ECHO.search(b):
            continue
        kept.append(b)
    return "\n\n".join(kept) if kept else (text or "").strip()


def format_inline_enumerations(text: str) -> str:
    """Склеенные «1) … 2) …» → список с пунктуацией."""
    s = (text or "").strip()
    if not s:
        return s
    s = re.sub(r"(?<!\n)(\d{1,2})\)\s+", r"\n\1. ", s)
    ...
    for ln in s.splitlines():
        t = ln.strip()
        if re.match(r"^\d{1,2}\.\s+", t) and t and t[-1] not in ".!?;:":
            t += "."
        lines.append(t)
    return "\n".join(lines)
...
def strip_reasoning_chatter(text: str) -> str:
    ...
    if not s:
        return s
    s = re.sub(r"(?is)<reasoning>.*?</reasoning>", "", s).strip()
    s = re.sub(r"(?is)\[thinking\].*?\[/thinking\]", "", s).strip()
    s = re.sub(r"(?is)```(?:think|reasoning)[\s\S]*?```", "", s).strip()
...
def no_reasoning_instruction() -> str:
    return (
        ...
        "симуляция диалога и «(Процесс…)». **Не пиши от имени Шефа** и не придумывай "
        "его следующий вопрос («Мне нужна помощь…», «У меня есть вопрос…»). "
        "Рассуждения — только если Шеф явно просит "
        "«объясни ход мыслей» / «подробно как ты думал».\n"
    )


def chat_communication_instruction() -> str:
    """Правила чата: переписка, не монолог модели."""
    return (
        "\n\n[Чат и коммуникация]\n"
        "Это **чат** с Шефом: одно сообщение — один готовый ответ. "
        "Не показывай черновики, планы, «во-первых я проверю…», «it seems», "
        "блоки рассуждений (think/reasoning) и пошаговый внутренний монолог. "
        "Не пересказывай вопрос целиком. "
        "Не симулируй реплики Шефа и не продолжай диалог за него. "
        "Краткость — сестра таланта: 1–3 предложения или список до 4 пунктов **с текстом в каждом**. "
        "Начал нумерованный список — **допиши все пункты**, не обрывай на «2.». "
        "Развёрнуто — только по явной просьбе («подробнее», «разверни»).\n"
    )
...
def truncate_for_voice_speech(text: str, user_text: str = "", *, max_sentences: int = 3) -> str:
    """Озвучка: не больше нескольких предложений, если Шеф не просил «подробнее»."""
    s = (text or "").strip()
    if not s or user_wants_expanded(user_text):
        return s
    parts = [p.strip() for p in _VOICE_SENTENCE_SPLIT.split(s) if p.strip()]
...
def _voice_conversational_trim(text: str, user_text: str = "") -> str:
    """Голосовой режим: короткая разговорная речь без markdown-украшений."""
    s = (text or "").strip()
    if not s:
        return s
    s = re.sub(r"\*\*([^*]+)\*\*", r"\1", s)
    s = re.sub(r"`([^`]+)`", r"\1", s)
    s = re.sub(r"!\[[^\]]*\]\([^)]+\)", "картинка в чате", s)
    s = truncate_for_voice_speech(s, user_text, max_sentences=3)
    if len(s) > 900:
        cut = s[:900].rsplit("\n", 1)[0]
        s = cut.rstrip() + "…"
    return s.strip()
...
def paragraph_format_instruction() -> str:
    return (
        ...
        "без пустой строки. "
        "**Перечисление** — только списком: каждый пункт с новой строки "
        "(«1. …» или «• …»), в конце пункта — точка или «;». Не склеивай пункты в одну строку. "
        "Не повторяй одну мысль разными словами. "
        "Не представляйся заново («Я — Jarvis…») — максимум один раз, если Шеф спросил кто ты. "
...
def length_instruction_for_prompt(user_text: str) -> str:
    ...
    if user_question_is_short(user_text):
        return (
            base
            + "[Длина] Вопрос в одно предложение — **кратко**: "
            "1–3 предложения в **отдельных абзацах** ИЛИ список до 4 пунктов с текстом в каждом. "
            "Начал список — допиши все пункты до конца. "
            "Запрещено повторять одну мысль, писать «---» и придумывать реплики Шефа.\n"
        )
    ...
    return base + "[Длина] Умеренно: 2–4 предложения в 1–2 абзацах, без воды; списки — целиком.\n"
...
def trim_incomplete_reply(text: str) -> str:
    """Починить markdown и хвост после «---»; **не** обрезать текст на полуслове."""
    s = (text or "").strip()
    if not s:
        return s
...
def apply_default_brevity(
    ...
    *,
    ...
    """Дедупликация и починка вёрстки; длину выбирает модель по промпту, не обрезка."""
    del max_sentences, max_chars  # совместимость сигнатуры
...
def polish_assistant_reply(
    ...
    *,
    ...
    try:
        ...
        if is_canned_smalltalk_reply(text or ""):
            return (text or "").strip()
    except Exception:
        pass

    s = _dedupe_repeated_sections(text or "")
    s = strip_identity_spam(s)
    s = format_inline_enumerations(s)
    s = strip_simulated_dialogue(s)
    s = strip_hallucinated_user_turns(s)
