...
STANDARD_UI_TOOLS_BRIDGE = (
        ...
        "- CORE TOOLS: Use available [web] and [rag] tools when necessary. Check dashboard indicators (Network, Tokens, Avito, Voice, DB) before making external actions.\n"
...
SYSTEM_PROMPTS = {
    AgentMode.STANDARD: (
        ...
        "- TOOLS: web_search (сниппеты), web_research (стереотипы → Google → выжимка, ссылки внизу), "
        "fetch_url (Chromium), memory_save_cell, UI/Avito; check LIVE sidebar before actions.\n"
        "- STEREOTYPES: файл **Стереотипы.txt** в бессознательном — приоритетные сайты для web_research.\n"
        "- CHAT: это переписка с Шефом — только готовый ответ, без внутренних рассуждений и черновиков.\n"
        ...
        "**Длина:** одно предложение от Шефа → 1–3 предложения или список до 4 пунктов; "
        "два и больше — можно развёрнуто. Не симулируй реплики Шефа. "
...
class AgentRuntime:
    ...
    chat_surface_mode: str = "text"
    last_chat_image_id: str = ""
    thinking_trace: list[str] = field(default_factory=list)
    dialog_session: dict = field(default_factory=dict)
    voice_reasoning_cache: list[dict] = field(default_factory=list)
...
def is_voice_interaction_mode() -> bool:
    """Голосовой диалог: микрофон «Джарвис» или режим чата voice."""
    return bool(_runtime.voice_enabled or _runtime.chat_surface_mode == "voice")
...
def build_llm_messages(user_text: str, history: list[dict]) -> list[dict]:
    ...
    return cloud_safe_messages(messages)


def cloud_safe_messages(messages: list[dict]) -> list[dict]:
    """Подготовка messages для облачных API — без пустых ролей и гигантских HTML-отчётов."""
    out: list[dict] = []
    for m in messages or []:
        ...
        if not content:
            continue
        if "<!-- jarvis-health-report -->" in content:
            content = "[Отчёт «Проверка систем Jarvis» — сводка скрыта для облака]"
        elif len(content) > 14_000:
            content = content[:14_000].rsplit("\n", 1)[0] + "\n…"
...
def build_system_message() -> str:
    ...
    if _runtime.mode == AgentMode.STANDARD:
        try:
            ...
            conscious_user = read_conscious_excluding_tech_doc(max_chars=2800)
        except Exception:
            conscious_user = ""
    ...
    try:
        from modules.user_preferences import preferences_layer_instruction, preferences_prompt_block

        extra += preferences_layer_instruction()
        prefs_block = preferences_prompt_block()
        if prefs_block:
            extra += f"\n\n[Предпочтения Шефа — игры, книги, каналы]\n{prefs_block}"
    except Exception:
        pass
    ...
    from modules.text_sanitize import chat_communication_instruction
    from modules.dialog_session import dialog_layer_instruction

    extra += chat_communication_instruction()
    extra += dialog_layer_instruction()
...
def build_qwen_system_message() -> str:
    ...
    qwen_role = (
        ...
        "Блок «Панели сайдбара — LIVE» выше — **текущее состояние** коннектора Авито; "
        "перед ответом про Авито сверяйся с ним и при необходимости вызывай инструменты. "
        "Ты работаешь с SQLite-метриками Авито. "
        "У тебя есть встроенный Chromium (fetch_url), web_research (Google → 10 страниц), "
        "web_search (сниппеты), UI-инструменты и SQLite Авито. "
        ...
        "**Длина ответа:** по умолчанию **очень коротко** (1–3 предложения), структурно; "
        "не обрывай фразу, не оставляй пустые пункты списка и не добавляй «---» с новым блоком; "
        "не пиши от имени Шефа; "
...
def _voice_should_use_deepseek(user_text: str) -> bool:
    """Голосовой режим: быстрый ответ через DeepSeek (не медиа/не пустые реплики)."""
    low = (user_text or "").strip()
    if len(low) < 3:
        return False
    try:
        from modules.dialog_handlers import is_acknowledgment_utterance, is_ambiguous_short_task

        if is_acknowledgment_utterance(low) or is_ambiguous_short_task(low):
            return False
    except Exception:
        pass
    try:
        ...
        if wants_media_generation(user_text) or wants_listing_generation(user_text):
            return False
    except Exception:
        pass
    return True
...
def chat_deepseek(api_key: str, model: str, user_text: str, history: list[dict]) -> tuple[str, int]:
    ...
    _runtime.log("llm", f"DeepSeek → {payload['model']}")
    with httpx_client(timeout=120.0, proxy=None) as client:
        try:
            ...
            if body:
                detail = f"{detail} | {str(body)[:480]}"
            _runtime.log("llm.error", f"DeepSeek HTTP: {detail}")
            raise
        data = resp.json()
...
def local_help_fallback(user_text: str, history: list[dict]) -> tuple[str, int]:
    ...
    elif any(w in low for w in ("браузер", "интернет", "запомн", "вспомн", "другом чат", "память", "jarvis", "кто ты")):
...
def _handle_doc_action(user_text: str, history: list[dict]) -> tuple[str, int]:
    ...
    if lq.is_local_setup_intent(user_text):
        low = user_text.lower()
        if any(w in low for w in ("логик", "bot_logic", "бот")):
            body = (
                "Шеф, модуль бота в этой сборке Jarvis отключён.\n"
                "Для интеграций используйте **коннектор Авито** или другие доступные инструменты в настройках."
            )
            ...
            body = (
                "Шеф, это настройка Jarvis (Настройки), не бухгалтерский документ. "
                "Уточните: Авито, ключ API или голос — подскажу по шагам."
            )
        _runtime.status = "IDLE"
    ...
    *,
    ...
    if _deepseek_key_valid(deepseek_key):
        try:
            ...
            _runtime.log("llm.error", f"DeepSeek fallback → Qwen: {e}")
            ...
            if lq.qwen_available():
                try:
                    ...
                    _runtime.log("llm.error", f"Qwen fallback failed: {inner}")

            fallback = (
                "Облачный ответ временно недоступен. Проверьте ключ DeepSeek в Настройках "
...
def route_and_generate(
    ...
    *,
    ...
    handled, meta_reply = try_handle_jarvis_meta_question(user_text)
    if handled:
        from modules.text_sanitize import polish_assistant_reply

        text = polish_assistant_reply(meta_reply, user_text, skip_brevity=True)
        _runtime.last_router_intent = "LOCAL_HELP"
        _runtime.last_router_engine = "meta_local"
        _runtime.status = "IDLE"
        return text, max(24, len(text) // 4)
    ...
    append_thinking(f"Роутер Qwen: классификация «{user_text[:56]}…»")
    ...
    try:
        ...
        if intent == "COMPLEX_TEXT" and is_jarvis_meta_question(user_text):
            intent = "LOCAL_HELP"
            engine = f"{engine}+meta_local"
    except Exception:
        pass
    _runtime.last_router_intent = intent
    _runtime.last_router_engine = engine
    _runtime.log("router", f"Интент **{intent}** ({engine})")
    append_thinking(f"Маршрут: **{intent}** · движок `{engine}`")
...
def _finish_reply(user_text: str, text: str, tokens: int) -> tuple[str, int]:
    """Обновить сессию диалога и runtime-трейс после готового текста чата."""
    from modules.dialog_session import update_dialog_session
    from modules.operation_stream import append_thinking

    try:
        update_dialog_session(user_text, text)
    except Exception:
        pass
    append_thinking(f"Ответ чата готов (~{len(text or '')} симв., ~{tokens} ток.)")
    return text, tokens
...
def generate_reply(
    ...
    *,
    ...
    append_thinking("Старт: анализ запроса и маршрутизация")

    try:
        from modules.dialog_session import prepare_user_turn

        user_text = prepare_user_turn(user_text, safe_history)
    except Exception:
        pass
    ...
    try:
        from modules.user_preferences import try_handle_remember_preference

        handled, pref_reply = try_handle_remember_preference(user_text)
        if handled:
            _runtime.status = "IDLE"
            _runtime.last_router_intent = "LOCAL_HELP"
            _runtime.last_router_engine = "user_prefs"
            from modules.text_sanitize import polish_assistant_reply

            text = polish_assistant_reply(pref_reply, user_text, skip_brevity=True)
            return _finish_reply(user_text, text, max(24, len(text) // 4))
    except Exception:
        pass

    try:
        from modules.user_preferences import enrich_user_text_with_preferences

        user_text = enrich_user_text_with_preferences(user_text, safe_history)
    except Exception:
        pass

    try:
        from modules.dialog_handlers import (
            try_handle_clarification_gate,
            try_handle_early_dialog,
        )

        handled, clarify_reply = try_handle_clarification_gate(user_text, safe_history)
        if handled:
            _runtime.status = "IDLE"
            _runtime.last_router_intent = "LOCAL_HELP"
            _runtime.last_router_engine = "clarify"
            from modules.text_sanitize import polish_assistant_reply

            text = polish_assistant_reply(clarify_reply, user_text, skip_brevity=True)
            return _finish_reply(user_text, text, max(24, len(text) // 4))
    ...
    try:
        ...
        if handled:
            ...
            if is_avito_report_message(avito_reply):
                return _finish_reply(user_text, avito_reply, max(24, len(avito_reply) // 4))
    ...
    if handled:
        _runtime.status = "IDLE"
        return _finish_reply(user_text, health_reply, max(1, len(health_reply) // 4))

    from modules.exchange_rates import try_handle_exchange_rate

    handled, rate_reply = try_handle_exchange_rate(user_text, safe_history)
    if handled:
        _runtime.status = "IDLE"
        return _finish_reply(user_text, rate_reply, max(24, len(rate_reply) // 4))
    ...
    if handled:
        _runtime.status = "IDLE"
        return _finish_reply(user_text, url_reply, max(1, len(url_reply) // 4))

    if is_voice_interaction_mode() and _deepseek_key_valid(
        deepseek_key
    ):
        if _voice_should_use_deepseek(user_text):
            try:
                from modules.operation_stream import append_thinking

                append_thinking("[VOICE] быстрый путь → DeepSeek (полный текст в чат)")
                text, tokens = chat_deepseek(deepseek_key, model, user_text, safe_history)
                _runtime.last_router_intent = "COMPLEX_TEXT"
                _runtime.last_router_engine = "deepseek_voice"
                _runtime.status = "IDLE"
                _runtime.log("router", "[VOICE] → DeepSeek")
                return _finish_reply(user_text, text, tokens)
            except Exception as e:
                _runtime.log("llm.error", f"Voice DeepSeek → router: {e}")

    try:
        text, tokens = route_and_generate(
            user_text,
            safe_history,
            deepseek_key,
            model,
