...
_VISIT_PROMISE_RE = re.compile(
    r"(?:приду|прийду|подъеду|заеду|приеду|буду у вас|приходите|ждите меня|"
    r"записывайте|до встречи|подтверждаю|согласен на встречу|согласна на встречу|"
    r"приду на собеседован|приду на встречу)",
    re.I,
)
...
_WHEN_RE = re.compile(
    ...
    r"в пятницу|в субботу|в воскресенье|\d{1,2}[.:]\d{1,2}(?:[.:]\d{2,4})?|"
    r"в \d{1,2}[:\s]\d{2})",
    re.I,
)


def _regex_extract_hr(text: str) -> dict[str, Any]:
    """Локальный разбор — без Qwen."""
    blob = (text or "").strip()
    if not blob or not _VISIT_PROMISE_RE.search(blob):
        return {"found": False, "method": "regex"}

    phone_m = _PHONE_RE.search(blob)
    when_m = _WHEN_RE.search(blob)
    phone = phone_m.group(0).strip() if phone_m else ""
    visit_at = when_m.group(0).strip() if when_m else ""

    lines = [ln.strip() for ln in blob.splitlines() if ln.strip()]
    candidate = ""
    for ln in lines:
        if _VISIT_PROMISE_RE.search(ln):
            candidate = ln.split(":")[0].strip()[:200]
            break
    if not candidate and lines:
        candidate = lines[0].split(":")[0].strip()[:200]
    ...
    if visit_at:
        confidence += 0.15
    if _VISIT_PROMISE_RE.search(blob):
        confidence += 0.1
...
def mine_hr_interviews(*, hours: int = 48) -> dict[str, Any]:
    ...
    for chat_id, msgs in by_chat.items():
        ...
        data = _regex_extract_hr(blob)
        if not data.get("found"):
            continue
        if float(data.get("confidence") or 0) < 0.72:
            llm_data = _llm_extract_hr(blob)
            if llm_data.get("found"):
                data = {**data, **{k: v for k, v in llm_data.items() if v}, "method": "regex+llm"}
