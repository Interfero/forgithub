...
export const ChatInput = forwardRef<ChatInputHandle, ChatInputProps>(
  function ChatInput(
    ...
    const attachHint =
      chatSurfaceMode === 'text' && !accountantMode
        ? 'Текст, PDF, изображения (jpg/png/webp) — до 15 МБ'
        : accountantMode
    ...
    const acceptTypes =
      chatSurfaceMode === 'text' && !accountantMode
        ? '.pdf,.txt,.docx,.md,.json,.jpg,.jpeg,.png,.webp,.gif,.bmp,audio/*,.ogg,.wav,.mp3,.webm,.m4a,.flac,.aac,.opus'
        : accountantMode
      ...
      <div className="border-t border-border bg-card/50 px-3 py-3 sm:px-4">
        ...
        <div className={cn(COMPOSER_GRID, 'mt-2')}>
          ...
          <p className="col-start-2 text-center text-[11px] text-muted-foreground">
            ...
            {chatSurfaceMode === 'text' && !marketerMode
              ? ' · Картинки: прикрепите файл или попросите обрезать/конвертировать'
              : chatSurfaceMode === 'voice'
                ? ' · Голос: переключатель и микрофон — слева над OpenConnect VPN'
                : ''}
            {marketerMode && chatSurfaceMode === 'text' ? ' · «нарисуй …» (Nano Banana)' : ''}
