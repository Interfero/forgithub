...
export interface Message {
  ...
  /** Готовое озвучивание с backend (SSE type: tts) */
  audioUrl?: string
  /** Короткий текст для TTS (не полный ответ чата) */
  speechText?: string
  /** Озвучка уже воспроизведена из стрима — не дублировать */
...
export interface ChatVoiceReadiness {
  ready: boolean
  edgeTts: boolean
  xttsReady: boolean
  sileroReady: boolean
...
export interface SttStatus {
  ...
  gigaamInstalled: boolean
  gigaamActive: boolean
  gigaamV3: boolean
  ffmpeg: boolean
  engine: string
  model: string
...
export interface OpenConnectVpnPreset {
  id: string
  label: string
  server: string
  port: number
...
export interface OpenConnectVpnConfig {
  server: string
  port: number
  username: string
  ...
  openconnectFound: boolean
  openconnectExe: string
  serverCertPinConfigured: boolean
  preset: OpenConnectVpnPreset
  ready: boolean
}

export interface OpenConnectVpnState {
  ...
  managed: boolean
  externalGui: boolean
  /** VPN на ПК вне Jarvis (OpenConnect GUI и т.п.) */
