...
export function AgentStatusIndicators({
  ...
  const br = agent.chromiumBrowser
  const gc = agent.googleChrome
  const browserOk = backendOk && br.ready && (!gc.requiredOnWindows || gc.ready)
  const browserWarn =
    backendOk &&
    (!browserOk ||
      br.installInProgress ||
      gc.installInProgress ||
  ...
  const coreWarn = backendOk && (!coreOk || browserWarn)
  const cloudActive =
    backendOk &&
    (ds.variant === 'success' ||
  ...
  const neuralLinks: NeuralLink[] = [
    ...
    { from: 'qwen', to: 'browser', kind: 'core', active: qwenActive && browserOk },
    {
      from: 'browser',
      to: 'agent',
      kind: 'core',
      active: browserOk && agentActive,
      warn: browserWarn,
    },
    { from: 'qwen', to: 'agent', kind: 'core', active: qwenActive && agentActive && !browserOk },
    {
      from: 'api-cloud',
      to: 'agent',
      kind: 'cloud',
        ...
        >
          ...
          <div className="min-h-0" style={{ gridColumn: 2, gridRow: '1 / span 5' }}>
            <CoreBlock
              title="Ядро Jarvis"
              subtitle={
                chatMode.variant === 'warning'
                  ? chatMode.hint
                  : 'Сервер · Qwen · Браузер · DeepSeek · источник ответа'
              }
              tone={coreTone}
            ...
            >
              ...
              <AnchoredChip anchor="browser">
                <JarvisBrowserStatusChip agent={agent} />
              </AnchoredChip>
