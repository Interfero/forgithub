...
interface DevPanelTelephonyProps {
...
export function DevPanelTelephony({ onSystemLog, onRefresh }: DevPanelTelephonyProps) {
  ...
  if (!cfg) {
    return (
      <p className="text-[10px] text-muted-foreground">Загрузка настроек АТС…</p>
    )
  }
  ...
  return (
    <div className="space-y-3 rounded-lg border border-border/60 bg-background/40 p-3">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs font-semibold">АТС — звонки на Джарвис</span>
      ...
      <p className="text-[9px] leading-relaxed text-muted-foreground">
        Входящий звонок на ваш номер в АТС → HTTP на Jarvis → озвучка приветствия (edge-tts).
        Для звонков из интернета укажите публичный URL (ngrok) и пропишите webhook в кабинете АТС.
      </p>

      <div className="grid gap-2 sm:grid-cols-2">
        <div>
          <label className="mb-1 block text-[10px] font-medium">Провайдер</label>
          <select
            value={cfg.provider}
        ...
        <div>
          ...
          <input
            ...
            className="w-full rounded-md border border-input bg-background px-2 py-1 font-mono text-[11px]"
          />
        </div>
      </div>

      <div className="space-y-1 rounded border border-dashed border-border/70 bg-muted/20 p-2 font-mono text-[9px]">
        <p>
      ...
      <div>
        <label className="mb-1 block text-[10px] font-medium">Секрет webhook (опционально)</label>
        <input
          type="password"
      ...
      <div>
        <label className="mb-1 block text-[10px] font-medium">Текст приветствия Джарвис</label>
        <textarea
      ...
      <details className="text-[10px]">
        ...
        <div className="mt-2 grid gap-2">
          <input
            ...
            className="rounded-md border border-input bg-background px-2 py-1 font-mono text-[11px]"
          />
          <input
            type="password"
