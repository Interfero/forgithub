...
import { TelegramTwinPanel } from '@/components/sidebar/TelegramTwinPanel'
  ...
  const blockProps = useMemo(
    () => ({
      ...
      voice: {
        ...
        description: 'XTTS-v2, студия голоса и озвучка ответов.',
      },
      telephony: {
      ...
      mail: {
        ...
        description: 'До 3 ящиков IMAP — логин и пароль пользователя.',
      },
      telegram: {
        id: 'settings-block-telegram' as const,
        blockId: 'telegram' as const,
        title: 'Коннектор Телеграм',
        description: 'BotFather, прокси и bot_logic.json.',
      },
      avito: {
          ...
          <div className="flex max-w-xl items-center gap-2">
            <div className="relative min-w-0 flex-1">
              ...
              <input
                ...
                placeholder="Название раздела, API, голос, Telegram…"
          ...
          <div className="min-h-0 min-w-0 flex-1 overflow-y-auto overscroll-y-contain px-5 py-4 [scrollbar-gutter:stable]">
              ...
              <div className="space-y-4">
                ...
                {!backendConnected && (
                  <p className="text-[10px] text-amber-700/90 dark:text-amber-300/90">
                    ...
                    <strong>{appBaseUrlWithSlash()}</strong>
                  </p>
                )}
                <div id="settings-section-deepseek" className="scroll-mt-4 border-t border-border/50 pt-4">
                  <ProviderCard
                    title="DeepSeek"
                    description={
                      agent.deepseekBundled
                        ? 'В Jarvis Free ключ встроен для друзей — менять не нужно.'
                        : 'Резерв для роутера и режима «Бухгалтер + Юрист». Только текст; картинки — Nano Banana в режиме «Маркетолог».'
                    }
                    serviceToggle={
                      !agent.deepseekBundled ? (
                        <ServiceEnableToggle
                  ...
                  >
                    {agent.deepseekBundled ? (
                      <p className="text-xs text-muted-foreground">
                        DeepSeek подключён (встроенный ключ). Дополнительные API — ниже в блоке
                        «Режимы и сервисы».
                      </p>
                    ) : (
                      <>
            ...
            >
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                <div id="settings-section-nanobanana" className="scroll-mt-4 space-y-3">
                  <Hint text="Google AI Studio — aistudio.google.com/apikey">
                    <ProviderCard
                      title="Google Nano Banana"
                      description="Генерация изображений через Gemini (один из медиа-провайдеров)."
                      serviceToggle={
                        <ServiceEnableToggle
            ...
            >
              <SettingsVoiceSection
                ...
                xttsActive={settings.xttsActive ?? true}
                onXttsActiveChange={(on) => void toggleService('xtts', on)}
                xttsServiceBusy={serviceBusy === 'xtts'}
                onXttsRefresh={() => onXttsRefresh?.()}
                onVoiceSlotUpdate={(s) => onVoiceSlotUpdate?.(s)}
