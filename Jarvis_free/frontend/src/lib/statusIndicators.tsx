...
export function StatusChip({ label, hint, value, variant, icon }: StatusChipProps) {
  return (
    <Hint text={hint}>
...
export function deepseekCloudChip(agent: AgentState): {
  ...
  if (agent.deepseekBundled && agent.deepseekUsable) {
    return {
      value: 'Встроен',
      variant: 'success',
      hint: 'DeepSeek встроен в Jarvis Free — облачные ответы доступны без своего ключа.',
      icon,
    }
  }
...
export function mailAccountChip(slot: MailAccountSlot, backendOk: boolean) {
  ...
  if (slot.status === 'ok') {
    return {
      ...
      hint: slot.lastEvent || `${slot.email} — подключение успешно.`,
      icon,
    }
  }
