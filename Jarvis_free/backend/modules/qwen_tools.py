...
18) **web_search** — поиск в интернете (DuckDuckGo, бесплатно).
   arguments: {"query": "текст запроса"}

19) **fetch_url** — открыть страницу во **встроенном Chromium** Jarvis (headless Playwright).
   arguments: {"url": "https://…"}
...
def maybe_auto_tools(
    ...
    if any(
        ...
        if not any(
            ...
            calls.append({"tool": "web_search", "arguments": {"query": q}})
    ...
    if remember_m:
        ...
        if len(fact) > 2:
            key = "шеф_" + re.sub(r"[^a-z0-9а-яё]+", "_", fact[:40].lower()).strip("_")[:32]
            key = key or "fact"
            calls.append(
                {
