...
def _mail_rows(mail: dict) -> list[Row]:
    ...
    for slot in mail.get("slots") or []:
        ...
        if st == "ok":
            rows.append((_level_ok(), title, slot.get("last_event") or "IMAP подключён"))
        elif st == "need_creds":
...
def _readiness_rows(
    ...
    if xtts.get("error") or "ошибк" in str(xtts.get("message") or "").lower():
        voice = (_level_err(), "Доступность голоса", xtts.get("message") or "Ошибка XTTS")
    elif x_st in ("installing_deps", "downloading_model", "downloading"):
        voice = (_level_warn(), "Доступность голоса", xtts.get("message") or "Установка XTTS…")
    elif (xtts.get("status") == "ready" or xtts.get("importable")) and voice_base.get("exists"):
        parts = ["XTTS", "образец голоса"]
        if rt.chat_speech_enabled:
            parts.append("озвучка ответов")
        if rt.voice_enabled:
            parts.append("микрофон")
        voice = (_level_ok(), "Доступность голоса", " · ".join(parts))
    elif voice_base.get("exists"):
        voice = (_level_warn(), "Доступность голоса", "Образец есть — донастройте XTTS и «Голос» в чате")
    else:
        voice = (_level_off(), "Доступность голоса", "Настройки → Голос и озвучка")
...
def collect_health_report_sections() -> dict[str, list[Row]]:
    ...
    if xtts.get("error") or "ошибк" in x_msg.lower():
        voice_block.append((_level_err(), "XTTS-v2", x_msg or "Ошибка движка"))
    elif xtts.get("status") in ("installing_deps", "downloading_model", "downloading"):
        voice_block.append((_level_warn(), "XTTS-v2", x_msg or "Загрузка…"))
    elif xtts.get("status") == "ready" or xtts.get("importable"):
        voice_block.append((_level_ok(), "XTTS-v2", x_msg or "Движок готов"))
    else:
        voice_block.append((_level_off(), "XTTS-v2", "Скачать в Настройках → Голос и озвучка"))
    if voice_base.get("exists"):
        voice_block.append((_level_ok(), "Образец голоса", voice_base.get("filename") or "загружен"))
    else:
        voice_block.append((_level_off(), "Образец голоса", "Загрузите WAV в Настройках → Голос"))
    ...
    from modules.jarvis_docs import probe_web_stack_parts

    for web_lv, web_name, web_det in probe_web_stack_parts():
        session.append((web_lv, web_name, web_det))

    tech_ok = any(
        (f.get("name") or "") == "Техдокументация.txt"
        for f in (mem.get("conscious") or [])
    )
    session.append(
        (
            _level_ok() if tech_ok else _level_warn(),
            "Техдокументация",
            "В сознательном — перечитывается каждые 10 мин"
            if tech_ok
            else "Нет Техдокументация.txt",
        )
    )
