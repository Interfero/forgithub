...
def classify_insult(
    ...
    *,
    ...
    if AT_JARVIS_RE.search(low):
        return "at_jarvis"
    if re.match(r"^(блять?|блин|чёрт|черт|ёб|еб)[\s,.!?]*$", low, re.I):
        return "venting"
...
def build_insult_turn_note(rt: Any) -> str:
    ...
    return (
        ...
        "Ответь **грубо и саркастично** (удар по самолюбию и соображалке, не матом-списком). "
        "Если в том же сообщении есть задача/вопрос — **сначала** дай решение по сути, "
        "потом или внутри ответа — колкости. Не игнорируй ни оскорбление, ни задачу."
    )
