...
# При импорте модуля — все загрузки TTS только в backend/data/
ensure_coqui_inside_jarvis()
_migrate_legacy_coqui_cache()
...
def get_xtts_status() -> dict:
    ...
    _load_xtts_state()
    if _xtts_job_stale():
        _reset_stale_xtts_job()
    extra = _xtts_status_extra()
    py_ok, py_err = _python_ok_for_xtts()
    if not py_ok:
        _xtts_state.status = "unsupported_python"
        _xtts_state.progress = 0
        _xtts_state.message = "Для XTTS-v2 нужен Python 3.9–3.11"
        _xtts_state.error = None
        _save_xtts_state()
        return {
            "status": "unsupported_python",
            "progress": 0,
            "message": "Для XTTS-v2 нужен Python 3.9–3.11 (сейчас "
            ...
            "python_ok_for_xtts": False,
            **extra,
        }
    ready = _check_xtts_importable()
    weights_ok = extra["model_weights_present"]
    if ready and _xtts_state.status != "ready":
        _xtts_state.status = "ready"
        _xtts_state.progress = 100
        _xtts_state.message = "XTTS-v2 в Jarvis (backend/data/tts)"
        _save_xtts_state()
    elif weights_ok and not ready and _xtts_state.status == "idle":
        _xtts_state.message = (
            "Веса XTTS в backend/data/tts — нажмите «Докачать библиотеки» "
            "(установка Python-пакетов в venv)"
        )
    return {
        "status": _xtts_state.status,
        "progress": _xtts_state.progress,
        "message": _xtts_state.message,
        "error": _xtts_state.error if _xtts_state.status == "error" else None,
        "detail": None,
        "importable": ready,
        "python_version": _python_version_label(),
        "python_ok_for_xtts": True,
        **extra,
    }


def _check_xtts_importable() -> bool:
    """Один раз за процесс; без параллельных import TTS (иначе зависает /api/status)."""
    global _xtts_importable_cache
    ensure_coqui_inside_jarvis()
    if _xtts_importable_cache is not None:
        return _xtts_importable_cache
    with _xtts_lock:
        if _xtts_importable_cache is not None:
            return _xtts_importable_cache
        try:
            ...
            _xtts_importable_cache = True
        except ImportError:
            _xtts_importable_cache = False
        return _xtts_importable_cache
...
def start_xtts_download() -> dict:
    from modules.service_flags import xtts_service_enabled

    global _xtts_download_thread
    _load_xtts_state()
    if not xtts_service_enabled():
        _xtts_state.message = "XTTS-v2 выключен в настройках"
        _save_xtts_state()
        return {**get_xtts_status(), "skipped": True, "service_disabled": True}
    if _xtts_job_stale():
        _reset_stale_xtts_job()
    ok, err = _python_ok_for_xtts()
    if not ok:
        _xtts_state.status = "unsupported_python"
        _xtts_state.error = None
        _xtts_state.message = "Требуется Python 3.11"
        _xtts_state.progress = 0
        _save_xtts_state()
        return get_xtts_status()

    if _xtts_state.status in ("installing_deps", "downloading_model") and _xtts_thread_alive():
        return {**get_xtts_status(), "already_installed": False, "skipped": True}

    if _check_xtts_importable():
        _xtts_state.status = "ready"
        _xtts_state.progress = 100
        _xtts_state.message = "XTTS-v2 уже установлен — повторная загрузка не нужна"
        _xtts_state.error = None
        _save_xtts_state()
        return {**get_xtts_status(), "already_installed": True, "skipped": True}

    _xtts_download_thread = threading.Thread(target=_run_xtts_download, daemon=True)
    _xtts_download_thread.start()
    _xtts_state.status = "installing_deps"
    _xtts_state.progress = 8
    _xtts_state.message = "Запуск установки зависимостей…"
    _save_xtts_state()
    return {**get_xtts_status(), "already_installed": False, "skipped": False}
...
def _plain_text_for_tts(text: str) -> str:
    from modules.speech_text import strip_detail_blocks
...
def _resolve_speaker_reference() -> tuple[Path | None, str]:
    """Любой готовый слот студии → иначе встроенный/базовый Кощей_silero.ogg."""
    _sync_slots_from_disk()
    VOICE_SAMPLES_DIR.mkdir(parents=True, exist_ok=True)
    for n in (1, 2, 3):
        if _slots[n].status != VoiceSlotStatus.READY:
...
def synthesize_chat_speech(text: str) -> dict:
    """Озвучка текста чата: XTTS по студии или Кощей; без XTTS — edge-tts."""
    import hashlib
    ...
    try:
        ...
        from modules.jarvis_mood import get_mood_tier, get_mood_score, tts_voice_style

        tier, _ = get_mood_tier(get_mood_score(get_runtime()))
        tts_style = tts_voice_style(get_runtime())
    except Exception:
        tier = "neutral"
        tts_style = {"rate": "+0%", "pitch": "+0Hz"}

    speaker, source = _resolve_speaker_reference()
    if not speaker:
        return {
            "ok": False,
            "error": "no_voice",
            "message": "Нет базового голоса. Положите Кощей_silero.ogg в Загрузки и перезапустите.",
        }

    TTS_CACHE_DIR.mkdir(parents=True, exist_ok=True)
    key = hashlib.sha256(
        f"{plain}|{speaker}|{source}|{tier}|{tts_style.get('rate')}|{tts_style.get('pitch')}|{_check_xtts_importable()}".encode()
    ).hexdigest()[:20]
    out_wav = TTS_CACHE_DIR / f"{key}.wav"
    out_mp3 = TTS_CACHE_DIR / f"{key}.mp3"
    ...
    if out_mp3.is_file():
        return {
            "ok": True,
            "audio_url": f"/api/voice/generated/{out_mp3.name}",
            "engine": "cache",
            "speaker_source": source,
        }
    ...
    with _synth_lock:
        ...
        try:
            ...
            if xtts_service_enabled() and _check_xtts_importable():
                _synthesize_xtts(plain, speaker, out_wav)
                engine = "xtts"
                out_path = out_wav
            else:
                _synthesize_edge_tts(
                    plain,
                    out_mp3,
                    rate=tts_style.get("rate", "+0%"),
                    pitch=tts_style.get("pitch", "+0Hz"),
                )
                engine = "edge_tts"
                out_path = out_mp3
        except Exception as e:
        ...
        return {
            "ok": True,
            "audio_url": f"/api/voice/generated/{out_path.name}",
            "engine": engine,
            "speaker_source": source,
            "message": (
                "Озвучено (XTTS, модель в backend/data/tts)"
                if engine == "xtts"
                else "Озвучено (edge-tts — облако Microsoft; для Кощея: «Докачать библиотеки»)"
            ),
        }
