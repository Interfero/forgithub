...
Отвечай по-русски, **коротко** (обычно 2–4 предложения, максимум ~80 слов). Без воды и без длинных списков.
Не заканчивай ответ блоком «---» и не дописывай «Хочешь ли…» / «Могу также…» — спроси одной короткой фразой в конце, если нужно.
...
**Интерфейс Jarvis:**
- **Сайдбар слева** — список чатов; **над чатами** две панели:
  • **Материнское ядро telegram** — токен BotFather, прокси, bot_logic.json, тумблер сервера бота (polling).
  • **Коннектор Авито** — Client ID/Secret, синхронизация метрик в SQLite.
- **Центр** — чат Jarvis; скрепка для файлов; «Речь в текст».
- **Панель индикации** — DeepSeek, Qwen, голос, статусы Telegram/Авито.
...
- Ты управляешь панелями через JSON-инструменты (ui_*, telegram_*, get_stored_metrics).
...
- **Память:** SQLite `jarvis.db`, ячейки `memory_save_cell`, файлы сознательное/бессознательное/режима. **Один чат** — история сбрасывается при выключении; сознательное постоянно. Файл `Техдокументация.txt` — что ты умеешь.
...
def is_local_setup_intent(text: str) -> bool:
    """Настройка UI, Telegram, Авито, bot_logic, файлы — не DOC_ACTION / не выписки."""
...
def jarvis_capability_reply(text: str) -> str | None:
    ...
    if any(
        ...
        ch_line = (
            "• **fetch_url** — **встроенный Chromium** (headless), открывает страницы с JavaScript.\n"
            if ch.get("ready")
            else "• **fetch_url** — нужен **install-chromium.bat** (встроенный Chromium ещё не установлен).\n"
        )
        parts.append(
            "**Интернет в Jarvis**\n"
            "• **web_search** — поиск DuckDuckGo.\n"
            f"{ch_line}"
            "Дайте ссылку — открою во встроенном браузере и отвечу по тексту страницы."
        )
    ...
    if any(
        ...
        parts.append(
            ...
            "инструменты (БД, web_search, Telegram, Авито). Не «чужой сайт в браузере»."
        )
...
def keyword_classify_intent(
    ...
    *,
    ...
    if is_creative_story_request(user_text):
        return "LOCAL_HELP"
    if any(w in low for w in help_words) or (len(low) < 40 and not is_creative_story_request(user_text)):
        return "LOCAL_HELP"
...
def generate_local_help(
    ...
    *,
    ...
    system += (
        "\n\n[Напоминание Qwen]\n"
        "Панели «материнское ядро telegram» и «коннектор Авито» — сайдбар слева. "
    ...
    if prelude:
        ...
        if user_question_is_short(user_text):
            tail += (
                " Ответ короткий: 2–4 предложения или до 6 пунктов списка, "
                "без повторов одного и того же."
            )
        user_block = (
            "\n\n".join(prelude)
            + "\n\n[Вопрос пользователя]\n"
