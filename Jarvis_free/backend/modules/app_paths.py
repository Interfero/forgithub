...
Jarvis Free: отдельные чаты/настройки (%LOCALAPPDATA%\\Jarvis_free), общие модели и браузеры с Pro.
"""
...
def shared_browsers_dir() -> Path:
    """Один каталог Chromium/Chrome для Pro и Free (%LOCALAPPDATA%\\Jarvis\\browsers)."""
...
def models_dir() -> Path:
    ...
    Free: общая папка Pro (../jarvis/backend/data/models), если есть.
    """
    if is_free_edition():
...
def user_data_dir() -> Path:
    ...
    Pro dev: backend/data
    Free: %LOCALAPPDATA%/Jarvis_free/data
    """
    ...
    if is_frozen():
        ...
        name = "Jarvis_free" if is_free_edition() else "Jarvis"
        return Path(base) / name / "data"
    if is_free_edition():
        base = os.getenv("LOCALAPPDATA") or os.getenv("APPDATA") or str(Path.home())
        return Path(base) / "Jarvis_free" / "data"
