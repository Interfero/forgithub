...
def read_store_text(disk_store: str, max_chars: int = 12000) -> str:
    store_key = disk_store_to_key(disk_store)
    with _connect() as conn:
        rows = conn.execute(
            """
...
def read_context_for_mode(mode_value: str, max_chars: int = 8000) -> dict[str, str]:
    """Контекст с изоляцией профессиональных режимов."""
    out: dict[str, str] = {
        "unconscious": read_store_text("unconscious", max_chars=max_chars),
        "conscious": "",
        "mode": "",
    }
    if mode_value == "standard":
        out["conscious"] = read_store_text("conscious", max_chars=max_chars)
...
def read_cells_text(mode_code: str | None, namespace: str = "default", max_chars: int = 4000) -> str:
    with _connect() as conn:
        if mode_code is None:
            rows = conn.execute(
                """
                SELECT cell_key, content FROM memory_cells
